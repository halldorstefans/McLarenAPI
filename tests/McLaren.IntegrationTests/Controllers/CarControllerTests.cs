...
namespace McLaren.IntegrationTests
{
    public class CarControllerTests : IntegrationTest
    {
        ...
        {
            var response = await _client.GetAsync("/api/f1/car");
        ...
        {
            var response = await _client.GetAsync("/api/f1/car/m7a");
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        {
            var response = await _client.GetAsync("/api/f1/car/m6a");
        ...
        {
            var response = await _client.GetAsync("/api/f1/car/1968");
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        {
            var response = await _client.GetAsync("/api/f1/car/1950");
            response.StatusCode.Should().Be(HttpStatusCode.NotFound);
        }
    }
}